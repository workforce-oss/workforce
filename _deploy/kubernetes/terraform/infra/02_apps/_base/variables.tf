...
variable "weaviate_enabled" {
  type        = bool
  description = "Whether or not weaviate is enabled."
}

variable "weaviate_secret_data" {
  type = object({
    namespace   = string
    secret_name = string
    secret_key  = string
  ...
  description = "Data to retrieve the weaviate token."

  default = {
    namespace   = "weaviate"
    secret_name = "weaviate-admin-api-key"
    secret_key  = "api_key"
  }
}
...
variable "workforce_admin_secret_data" {
  ...
  description = "Data to create the workforce admin secret."
